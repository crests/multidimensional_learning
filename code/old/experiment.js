...
function showResponse(hit_or_blow){
  ...
  for (var i=0; i<resultPtag.length; i++){
    resultPtag[i].innerHTML = resultTxt;
  }
  showScore();
}
function showScore(){
...
function getHistoryArray(){
  return [History.left, History.right, History.choice, History.result, History.score];
}

function getTrialHistoryArray(){
  return [History.left, History.right, History.probLeft,History.probRight,History.choice, History.result, History.score];
}
...
function getCSV(){
  ...
  txt.open('get', "file:///Users/arataNonami/research/feature-learning/code/randomArr.csv", false); //同期リクエスト
  txt.send();
...
function getProbability(){
  var index = History.left.length - 1;
  var left = History.left[index];
  var right = History.right[index];

  var e = Math.E;
  var div = 80.0;
  var wn = Math.pow(e,(FeatureParam[0][index]/div));
  var ws = Math.pow(e,(FeatureParam[1][index]/div));
  var wc = Math.pow(e,(FeatureParam[2][index]/div));

  var diff_n = (left[0] - right[0]) * 0.5 + 0.5;
  var diff_s = (left[1] - right[1]) * 0.5 + 0.5;
  var diff_c = (left[2] - right[2]) * 0.5 + 0.5;
  ...
  var p_left = wn2*diff_n + ws2*diff_s + wc2*diff_c;
  var p_right = wn2*(1-diff_n) + ws2*(1-diff_s) + wc2*(1-diff_c);

  History.probLeft.push(p_left);
  History.probRight.push(p_right);
